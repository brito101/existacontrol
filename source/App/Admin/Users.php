...
class Users extends Admin {
    ...
    public function home(?array $data): void {
        ...
        if (!empty($data["s"])) {
            $s = str_search($data["s"]);
            echo json_encode(["redirect" => url("/admin/usuarios/home/{$s}/1")]);
            return;
        }
        ...
        if (!empty($data["search"]) && str_search($data["search"]) != "all") {
            ...
            if (!$users->count()) {
                ...
                redirect("/admin/usuarios/home");
            }
        }

        $all = ($search ?? "all");
        $pager = new Pager(url("/admin/usuarios/home/{$all}/"));
        ...
        echo $this->view->render("widgets/users/home", [
            ...
            "head" => $head,
            "search" => $search,
            "users" => $users->order("created_at DESC, first_name, last_name")->limit($pager->limit())->offset($pager->offset())->fetch(true),
    ...
    public function user(?array $data): void {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            $json["redirect"] = url("/admin/usuarios/usuario/{$userCreate->id}");
