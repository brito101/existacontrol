...
class Auth extends Model
{
    ...
    public function attempt(string $email, string $password, int $level = 1): ?User
    {
        ...
        $subscription = (new AppSubscription())->find("user_id = :user_id AND status = 'canceled'", "user_id={$user->id}");
        if ($subscription->count()) {
            $this->message->error("Desculpe, mas sua conta foi cancelada");
            return false;
        }

        $subscriptionCondominium = (new AppSubscriptionCondominium())->find("user_id = :user_id AND status = 'canceled'", "user_id={$user->id}");
        if ($subscriptionCondominium->count()) {
            $this->message->error("Desculpe, mas sua conta foi cancelada");
            return false;
        }
